...
export function Deltaphoto({
  ...
  const startSmoothing = useCallback(() => {
    ...
    const step = (timestamp: number) => {
      ...
      // The time-based easing keeps the drag feeling consistent across displays.
  ...
  const handleChange = useCallback((nextPosition: number) => {
  ...
  const handleKeyDown = useCallback((event: KeyboardEvent<HTMLInputElement>) => {
    ...
    const nextPosition = event.key === "ArrowLeft"
      ? currentPosition <= 50 ? 0 : 50
      : currentPosition >= 50 ? 100 : 50;
  ...
  return (
    <div
      className={`deltaphoto ${className}`.trim()}
      style={rootStyle}
      {...rest}
    >
      <img
